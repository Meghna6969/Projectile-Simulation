 ...
 */
let scene, camera, renderer, controls; // General 3D Items
let perspectiveCamera, orthographicCamera; // Stores 2D and 3D camera for switch when requested
let isCurr2D = false; // Keeps track of the current view of the scene
let gridHelper;
let cannonModel;
let shotCounter = 0; // Keeps track of the trajectory color with the shooter
...
let mass = 5; // Per ball basis also initial mass of the object

// This thing is stupid
// Physics Variables for drag and air resistence
...
let maxHeight = 0;
let dataPoints = []; // Details panel information
let trajectoryLines = [];
const MAX_TRAJECTORY_LINES = 3; // Max trajectory lines in a given scene, each have their own color

// CANNON.js Setup for rigidbody physics
let world;
let physicsBodies = [];
let physicsMeshes = [];
...
toggle.checked = true; // Initially 3d view

// All the inputs in the scene
...
// Binding slider with the input so they have same information
...
airResis.addEventListener('input', (e) => {
    ...
    if (airResistent) {
        enableAirResistence();
        // Devs only; just incase if the air resistence is not visible in the scene due to factors this will let you know if its on or off
...
// Helper function that is can create boxes with ease, binds the physics object as well
    ...
    physicsBodies.forEach(body => {
        if (airResistent && body.velocity) {
            ...
            if (speed > 0.01 && body.mass > 0) {
                ...
                // Drag calculation per object basis as mass of the objects change
...
// Gimbal lock with the cannon and making sure the rotation is always with respective of the scene
function updateRotationLocks() {
    if (isCurr2D) {
        ...
        // Disabling the z and y rotation sliders when in 2D
...
// Using Euler rotation for accurate rotation of the cannona
...
function createBackground() {
    ...
    // Creating CANNON.js imposter for the rigidbody physics
...
function setupThreeScene() {
    ...
    scene.background = new THREE.Color(0x140f2e); // Purple background color
...
// Cannon main element here
function interactiveObjects() {
    const loader = new THREE.GLTFLoader();
    loader.load('assets/cannon.glb', function (gltf) {
...
// Bunch of physics with launching the projectile
function launchProjectile() {
    const v0 = parseFloat(velocityInput.value); // Initial Velocity
    const h0 = parseFloat(heightInput.value) + 1; // Initial Height

    const radius = 0.5 * mass * 0.1; // * 0.1 to basically normalize the ball
    const direction = new THREE.Vector3(0, 1, 0); // Initial Vector
    const euler = new THREE.Euler(cannonRotationX, cannonRotationY, cannonRotationZ, 'YXZ'); // Euler Rotation so we stay consistent with the whole scene
    direction.applyEuler(euler);
    direction.normalize(); // Just so the Vector dont go crazy

    const barrelLength = 1; // From the initial point
...
// Trajectory creator (Only Creates them!!!); Only creates trajectory until the object collides the ground and stops the timer as well
...
// Updates the created trajectory based on the current location of the projectile for every trajectory object
